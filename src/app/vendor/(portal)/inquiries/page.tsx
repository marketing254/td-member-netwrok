...
const PARTNER_WINE_DARK = "#4A2030";
const PARTNER_WINE_TINT = "#F4E7EB";
const INK = "#0A1A2F";
  ...
  return (
    <Stack spacing={3.5} sx={{ maxWidth: 920, mx: "auto" }}>
      <Box>
        ...
        >
          Member questions on your resources
        </Typography>
        <Typography
          sx={{
            color: INK_SOFT,
        ...
        >
          Every inquiry a member posts on a resource you published shows up
          here. Reply directly — the whole thread is visible to every member
          viewing that resource, so a good answer builds your reputation in
          the network.
        </Typography>
      </Box>
      ...
      >
        {loading ? (
          <Stack sx={{ alignItems: "center", py: 6 }}>
            <CircularProgress size={22} sx={{ color: PARTNER_WINE }} />
          </Stack>
        ) : rows.length === 0 ? (
          <Box sx={{ p: { xs: 4, md: 6 }, textAlign: "center" }}>
            ...
            >
              When a member asks a question on one of the resources you
              published, it lands here. You&apos;ll also get a notification.
            </Typography>
          </Box>
        ) : (
  ...
  return (
    <Chip
      ...
      sx={{
        height: 20,
        fontSize: "0.7rem",
        bgcolor: active ? PARTNER_WINE_TINT : "grey.100",
        color: active ? PARTNER_WINE_DARK : INK_MUTED,
        fontWeight: 700,
      }}
    />
  );
    ...
    <Box sx={{ bgcolor: expanded ? "grey.50" : "transparent" }}>
      <Box
        ...
        sx={{
          ...
          "&:focus-visible": {
            outline: `2px solid ${PARTNER_WINE}`,
            outlineOffset: -2,
          },
        }}
      >
          ...
          >
            {loadingReplies ? (
              <Stack sx={{ alignItems: "center", py: 2 }}>
                <CircularProgress size={18} sx={{ color: PARTNER_WINE }} />
              </Stack>
            ) : (replies ?? []).length === 0 ? (
              <Typography
                sx={{
            ...
            <Stack direction="row" spacing={1} sx={{ justifyContent: "flex-end" }}>
              <Button
                ...
                sx={{
                  ...
                  borderRadius: 999,
                  px: 2.5,
                  bgcolor: `${PARTNER_WINE} !important`,
                  backgroundImage: `linear-gradient(180deg, ${PARTNER_WINE} 0%, ${PARTNER_WINE_DARK} 100%) !important`,
                  color: "#FFFFFF !important",
                  "&:hover": {
                    bgcolor: `${PARTNER_WINE_DARK} !important`,
                    backgroundImage: `linear-gradient(180deg, ${PARTNER_WINE} 0%, ${PARTNER_WINE_DARK} 100%) !important`,
                    color: "#FFFFFF !important",
                  },
                  "&.Mui-disabled": {
                    bgcolor: "rgba(110,51,70,0.3) !important",
    ...
    <Stack direction="row" spacing={1.25} sx={{ alignItems: "flex-start" }}>
      <Avatar
        sx={{
          width: 30,
          height: 30,
          bgcolor: isMine ? PARTNER_WINE_TINT : GOLD_SOFT,
          color: isMine ? PARTNER_WINE : GOLD,
          fontSize: "0.7rem",
          fontWeight: 700,
          flexShrink: 0,
          mt: 0.25,
        ...
        <Stack direction="row" spacing={0.85} sx={{ alignItems: "center", flexWrap: "wrap" }}>
          ...
          <Chip
            ...
            sx={{
              height: 16,
              bgcolor: isMine ? PARTNER_WINE_TINT : GOLD_SOFT,
              color: isMine ? PARTNER_WINE_DARK : GOLD,
              fontSize: "0.58rem",
              fontWeight: 800,
              textTransform: "uppercase",
...
function StatusChip({ status }: { status: Inquiry["status"] }) {
  const map: Record<Inquiry["status"], { label: string; bg: string; color: string }> = {
    ...
    answered: { label: "Answered", bg: PARTNER_WINE_TINT, color: PARTNER_WINE_DARK },
